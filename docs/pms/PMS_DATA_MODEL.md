...
2. **Building positions from orders**  
   Positions are at **(account_id, book, asset)** grain. For each order (filter: partially_filled, filled), derive base and quote from symbol; post **base leg** (+/??executed_qty to base_asset) and **quote leg** (+/??quote amount to quote_asset). Group by (account_id, book, asset); add **balance_changes** net delta per (account_id, book, asset). **open_qty** = order_derived + balance_changes sum; **position_side** = long | short | flat; **entry_avg** from order-derived FIFO only.
...
## Building positions from orders and balance_changes

PMS **derives positions at asset grain** from **orders** (base + quote legs) and **balance_changes**:

- **Orders:** Filter by status **partially_filled**, **filled**. For each order, resolve symbol ??(base_asset, quote_asset) from **symbols**. Post **base leg** (signed executed_qty) and **quote leg** (signed quote amount, e.g. from binance_cumulative_quote_qty or executed_qty ? price) to (account_id, book, asset). Group by (account_id, book, asset); run FIFO per asset to get order_derived open_qty and entry_avg.
- **Balance_changes:** Filter by change_type IN (deposit, withdrawal, adjustment, transfer). Sum delta per (account_id, book, asset).
- **Final position:** open_qty = order_derived_open_qty + SUM(balance_changes.delta) per (account_id, book, asset). position_side = long | short | flat from sign of open_qty.
- **Use in PnL:** Realized PnL from executed order rows; unrealized PnL = (mark ??entry_avg) ? open_qty when open_qty ??0. **USD valuation** uses the **assets** table (usd_price for stables; usd_symbol for future mark fetch). See **docs/pms/POSITION_KEEPER_DESIGN.md** and **docs/pms/REFACTORING_PLAN_POSITIONS_AS_ASSETS.md**.
...
## All coins are positions (asset grain)

- **Positions** = open exposure per **asset** per (account_id, book). Grain **(account_id, book, asset)** with **open_qty** (signed) and **position_side** ('long' | 'short' | 'flat'). Every coin (BTC, USDT, ETH, etc.) is a position; there is no separate cash table. Build = **orders** (base + quote legs via symbols) + **balance_changes** (deposit/withdrawal/transfer/adjustment).
- **USD valuation** uses the **assets** table: per-asset **usd_price** (e.g. 1 for stables) or **usd_symbol** for mark fetch. Notional and unrealized PnL in USD are set from this config. See **docs/pms/POSITION_KEEPER_DESIGN.md** and **docs/pms/REFACTORING_PLAN_POSITIONS_AS_ASSETS.md**. A combined holdings view (all assets) is the positions table itself; capital by book = sum of position values (open_qty x mark) per book.
...
| **Building positions** | PMS derives positions at **(account_id, book, asset)** from **orders** (base + quote legs) + **balance_changes**; filter orders partial/fully filled; entry_avg from FIFO. **USD valuation** via **assets** table (usd_price / usd_symbol). |
