...
- **Testnet:** Use Binance testnet for API keys and base URL; no real funds. For API key, signature, and symbol filter rules (e.g. -2014, -1022, -1013), see **docs/oms/BINANCE_API_RULES.md**.
...
- [ ] **12.3.2** **(Core)** **PMS Postgres schema** (reads): orders, **balance_changes**, **symbols**, accounts. **Orders are the primary data source** (no fills table). In build-from-order model, **balance sync disabled**; PMS does not read `balances` for cash. **Symbols** (reference, OMS-managed) for base/quote. Grain includes **book**. Document in **docs/pms/PMS_ARCHITECTURE.md** §8.
...
- [x] **12.5.4** **Risk deployment (optional Phase 2):** Docker service `risk`; env `REDIS_URL` and rule config; loop strategy_orders → risk → risk_approved. **E2E with Risk in path:** `scripts/e2e_with_risk.py` — injects to strategy_orders, times each event (inject → risk_approved → oms_fill → orders table) and per-module latency, checks downstream data integrity (risk_approved, oms_fills, Postgres orders). Test inject can still target `risk_approved` when Risk is disabled.
...
│   ├── account_sync.py      # sync accounts/balances to Postgres; write_balance_change (with book)
│   ├── symbol_sync.py       # (optional) sync symbols table from broker exchangeInfo
...
│   ├── Dockerfile
│   ├── requirements.txt
│   ├── cash.py             # (optional) build_book_cash_from_orders_and_balance_changes
│   └── main.py             # loop: read orders/balance_changes/symbols → positions + book_cash → Postgres
├── risk/
│   ├── Dockerfile
│   ├── requirements.txt
...
- **PMS build-from-order cash:** Unit test `build_book_cash_from_orders_and_balance_changes` (mock orders, balance_changes, symbol_map; assert aggregated balances). Unit test `query_balance_changes`, `query_symbols` / `get_symbol_map` (mock Postgres). See tasks 12.3.9–12.3.15.
...
| **PMS data source** | §8.1: read "orders, balance_changes, symbols" | In **build-from-order** model: position derivation from **orders**; cash built from **orders** + **balance_changes**; **balance sync disabled** (PMS does not read `balances` for cash). |
| **PMS writes** | §8.1: "positions, book_cash, pnl_snapshots, Redis" | **positions** table (with unrealized_pnl per row); **book_cash** (grain account_id, book, asset) in build-from-order model. No Redis pnl/margin keys, no pnl_snapshots/margin_snapshots (optional). |
