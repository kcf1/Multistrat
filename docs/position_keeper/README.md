# Position Keeper — Docs

Position Keeper reads from OMS Postgres/Redis, computes PnL and margin, and writes snapshots. This folder documents design notes and data-model considerations for the service.

---

## References

- **Phase 2 plan:** [PHASE2_DETAILED_PLAN.md](../PHASE2_DETAILED_PLAN.md) §8 (Position Keeper role), §12.3 (Position Keeper tasks).
- **OMS schema:** Postgres `orders`, `accounts`, `balances`, `balance_changes`; OMS does **not** have a Postgres `positions` table (positions live in Redis only). See [AMS_DB_FIELDS.md](../ams/AMS_DB_FIELDS.md), [PHASE2_DETAILED_PLAN.md](../PHASE2_DETAILED_PLAN.md) §3.1.
- **Fills:** Source of truth for executions is downstream of OMS (e.g. Position Keeper consumes `oms_fills` and may write to a `fills` table). OMS does not write to `fills`.
...
### Implications for Position Keeper
...
Document the convention or mapping you choose (e.g. in this README or in `position_keeper` code) so reconciliation logic is clear.

---

## Building positions from orders (and fills)

OMS does **not** expose a Postgres **positions** table. Positions are held in Redis only (`account:{broker}:{account_id}:positions`), populated by OMS from broker account streams.

Position Keeper may still need to:
...
Position Keeper will often need to reconcile **two sources of truth**:
...
Document which of these reconciliations you implement, where (e.g. Position Keeper job, script, or admin tool), and how often (e.g. periodic job, on-demand).
...
| **Building positions** | OMS has no Postgres positions table. Position Keeper can build positions from orders/fills (e.g. by account, symbol, side) and optionally reconcile with Redis/broker positions. |
...
Implement helpers or mapping (code or DB) as needed and document them here or in `position_keeper` so future work stays consistent.
