...
def run_repair_taker_buy_sell_volume_gaps_policy_window_all_series(
    ...
    *,
    ...
    provider_executor: ProviderExecutor[PolicyTakerBuySellVolumeRepairSeriesResult] | None = None,
    ...
    if ex is None:
        ex = ProviderExecutor[PolicyTakerBuySellVolumeRepairSeriesResult](
    ...
    def _run_task(symbol: str, period: str) -> PolicyTakerBuySellVolumeRepairSeriesResult:
        ...
        try:
            ...
            if not gaps:
                return PolicyTakerBuySellVolumeRepairSeriesResult(symbol, period, 0, 0)
            ...
            return PolicyTakerBuySellVolumeRepairSeriesResult(symbol, period, len(gaps), n)
        finally:
            conn.close()
    ...
    try:
        ...
        logger.info(
            "repair_gap_taker_buy_sell_volume run start: tasks={} workers={}",
            len(tasks),
            ex.max_workers,
        )
        if ex.max_workers <= 1:
            ...
            logger.info(
                "repair_gap_taker_buy_sell_volume run done: submitted={} completed={} failed=0 wall_clock_s={:.3f}",
                len(tasks),
                len(tasks),
        ...
        futures: list[Future[PolicyTakerBuySellVolumeRepairSeriesResult]] = []
        future_to_task: dict[Future[PolicyTakerBuySellVolumeRepairSeriesResult], tuple[str, str]] = {}
        for symbol, period in tasks:
        ...
        for fut in futures:
            ...
            try:
                ...
                logger.exception(
                    "repair_gap_taker_buy_sell_volume task failed: symbol={} period={}",
                    symbol,
                    period,
                )

        logger.info(
            "repair_gap_taker_buy_sell_volume run done: submitted={} completed={} failed={} wall_clock_s={:.3f}",
            len(tasks),
        ...
        if failed_tasks:
            ...
            raise RuntimeError(
                "repair_gap_taker_buy_sell_volume failed for task(s): "
