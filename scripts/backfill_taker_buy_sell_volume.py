#!/usr/bin/env python3
"""
One-shot taker buy/sell volume ingest for Postgres ``taker_buy_sell_volume``.
...
from market_data.jobs.ingest_taker_buy_sell_volume import (  # noqa: E402
...
from market_data.storage import taker_buy_sell_volume_window_stats  # noqa: E402
...
def _log_summary(conn, *, end_ms: int, backfill_days: int, results: list) -> None:
    ...
    for symbol in TAKER_BUYSELL_VOLUME_SYMBOLS:
        for period in TAKER_BUYSELL_VOLUME_PERIODS:
            ...
            stored, oldest, newest = taker_buy_sell_volume_window_stats(
                conn,
                symbol,
                period,
    ...
    print("\n=== Taker Buy/Sell Volume completeness (policy window, approximate) ===", flush=True)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Taker buy/sell volume backfill / catch-up into Postgres."
    )
    ...
    parser.add_argument(
        "--symbols",
        default=None,
        help="Optional comma-separated symbols to run (overrides default TAKER_BUYSELL_VOLUME_SYMBOLS).",
    )
    ...
    logger.info(
        "Taker buy/sell volume ingest: symbols={} periods={} backfill_days={} use_watermark={} skip_existing_when_no_watermark={}",
        TAKER_BUYSELL_VOLUME_SYMBOLS,
        TAKER_BUYSELL_VOLUME_PERIODS,
        backfill_days,
        use_watermark,
    ...
    symbols = TAKER_BUYSELL_VOLUME_SYMBOLS
    if args.symbols:
