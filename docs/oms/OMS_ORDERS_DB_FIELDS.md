...
**Post-sync repairs:** `oms/repair.py` — `run_all_repairs` runs periodically (after sync in the OMS loop) and fixes flawed values for `broker = 'binance'` orders by recovering from the `payload` JSONB: `price` (from avgPrice or fills[0].price when NULL/0), `time_in_force` (when NULL/empty), `binance_cumulative_quote_qty` (when NULL), `status` (from payload.binance.status or .X whenever payload has it; no null check on current DB status).
